...
WHERE
    EXISTS (
        SELECT 1
        FROM jsonb_array_elements (t.hashtags -> 'hashtags') elem
        WHERE
            upper(elem ->> 'tag') IN (
                'ECONOMIA',
                'ACIONISTAS',
...
INSERT INTO
    collection_log (search_term)
VALUES (
        '{"x_user_id": "51150679", "to_date_time": "2026-02-12T03:00:00Z", "from_date_time": "2025-09-01T02:59:00Z"}'
    );
...
SELECT MAX(t.created_at), MIN(t.created_at)
FROM tweets t
ORDER BY created_at DESC;

SELECT t.*, EXISTS (
        SELECT 1
        FROM tweets_classification tc
        WHERE
            tc.tweet_id = t.id
...
SELECT * FROM tweets_classification c ORDER BY tweet_id DESC;
