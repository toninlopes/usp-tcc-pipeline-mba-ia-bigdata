...
Produz o modelo ajustado em models/bert-timbau-sentiment/, que é então usado
pelo BERTimbauAnalyzer via processing_dashboard.
...
from shared.text_cleaner import (  # noqa: E402
    remove_emojis,
...
OUTPUT_DIR = Path(__file__).resolve().parents[2] / "models" / "bert-timbau-sentiment"
MAX_LENGTH = 128
RANDOM_STATE = 42

LABEL_TO_ID = {"negativo": 0, "neutro": 1, "positivo": 2}
...
def preprocess(text: str) -> str:
    """Limpeza textual para BERTimbau cased. Preserva capitalização para não
    destruir tickers (PETR4), siglas (COPOM) e nomes próprios."""
...
class TweetDataset(Dataset):
    """PyTorch Dataset empacotando encodings tokenizados + labels inteiros."""
...
# ── Weighted Trainer (lida com desbalanceamento) ──────────────────────────────
...
def load_labeled_data() -> pd.DataFrame:
    ...
    db = DatabaseManager()
    rows = db.query_all_tweets_with_human_classification()
    df = pd.DataFrame(
        rows,
        columns=[
            "id",
...
def main() -> None:
    ...
    train_df[["id", "tweet_id", "sentiment"]].to_csv(
        eval_dir / "train_split.csv", index=False
    )
    val_df[["id", "tweet_id", "sentiment"]].to_csv(
        eval_dir / "val_split.csv", index=False
    )
    test_df[["id", "tweet_id", "sentiment"]].to_csv(
        eval_dir / "test_split.csv", index=False
    )
